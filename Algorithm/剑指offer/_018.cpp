...
void CreateListT(LinkList *&L,ElemType a[],int n)//尾插法
{
    LinkList *s,*r;
    int i;
...
int GetELem(LinkList *L,int i,ElemType &e){//打印出某一元素
    int j=0;
    LinkList *p=L;
    while(j<i&&p!=NULL)
    {
...
void  Delete_2(LinkList *&L,LinkList* pToBeDeleted)
{
    LinkList *pListHead=L;
    if(!(pListHead)||!pToBeDeleted)
        return;
    if(pToBeDeleted->next!= nullptr)//要删除的节点不是尾节点
    {
        LinkList* pnext=pToBeDeleted->next;//将要删除节点的next节点赋值给要删除节点
    ...
    //当链表只有一个节点，删除的即是头节点也是尾节点
    else if(pListHead ==pToBeDeleted)
    {
        delete pToBeDeleted;
        pToBeDeleted= nullptr;
        pListHead= nullptr;//在删除节点后，还需将头节点设置为nullptr


    }
    else
    {
        //要删除的节点在链表的尾部,只能够顺序遍历到该节点的前序节点处，并删除
